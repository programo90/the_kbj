					...
					<div class="container-fluid">
						<div class="row">
							<div class="col-md-12" id="con-row">
								<div class="row">
									<div class="col-md-12">
										<form role="form" action="sportsModifyresult.do" method="post"
											name="frm">
											...
											<input type="hidden" class="form-control" id="mno" name="mno"
												value="${requestScope.dto.bno}" />
											<div class="form-group" id="mnick-box">
												<label for="mnick"> ${requestScope.dto.mnick} </label> <input
													type="hidden" class="form-control" id="mnick" name="mnick"
													value="${requestScope.dto.mnick}" />
											</div>
											<hr>
											<div class="form-group">
												<label for="bctg">카테고리</label> <br> <select
													id="bctg" name="bctg" onchange="select_tag_sel(this)">
											...
											<div class="form-group">
												<label for="btag">태그</label> <br> <select
													id="btag" name="btag">
													<c:choose>
														<c:when test="${requestScope.dto.btag == '축구'}">
															<option value="축구" selected="selected">축구</option>
															<option value="해외축구">해외축구</option>
															<option value="야구">야구</option>
															<option value="해외야구">해외야구</option>
														</c:when>
														<c:when test="${requestScope.dto.btag == '해외축구'}">
															<option value="축구">축구</option>
															<option value="해외축구" selected="selected">해외축구</option>
															<option value="야구">야구</option>
															<option value="해외야구">해외야구</option>
														</c:when>
														<c:when test="${requestScope.dto.btag == '야구'}">
															<option value="축구">축구</option>
															<option value="해외축구">해외축구</option>
															<option value="야구" selected="selected">야구</option>
															<option value="해외야구">해외야구</option>
														</c:when>
														<c:when test="${requestScope.dto.btag == '해외야구'}">
															<option value="축구">축구</option>
															<option value="해외축구">해외축구</option>
															<option value="야구">야구</option>
															<option value="해외야구" selected="selected">해외야구</option>
														</c:when>
													</c:choose>
												</select>
											</div>
											...
											<div style="text-align: center;">
												<div class="checkbox" id="robot">
													...
													<input type="hidden" value="${requestScope.dto.bimg}" id="bimg" name="bimg"> 
												</div>
											</div>
